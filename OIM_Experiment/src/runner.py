...
from OIM_Experiment.src.OIM_Simple import OIMMaxCut
from OIM_Experiment.src.graph_utils import read_graph, random_graph, verify_cut
from OIM_Experiment.src.visualization import MaxCutVisualizer
from OIM_Experiment.src.experiment_logger import MaxCutExperimentLogger


def get_args():
    parser = argparse.ArgumentParser(description='OIM Max-Cut Solver')
    parser.add_argument('--K', type=float, default=1.0,
                        help='Oscillator coupling strength (default 1.0)')
    parser.add_argument('--Ks', type=float, default=2.0,
                        help='Sub-Harmonic Injection Locking strength (default 2.0). '
                             'Penalty coefficient lambda = Ks/K. '
                             'Only used with --coupling cosine_shil.')
    parser.add_argument('--coupling', type=str, default='cosine_shil',
                        choices=['cosine_shil', 'generalized'],
                        help='Coupling mode: cosine_shil (Eq. 8, approx ratio 0.878) '
                             'or generalized (Theorem 6, approx ratio 1.0).')
    ...
    parser.add_argument('--init_mode', type=str, default='small_random',
                        ...
                        help='Initialisation strategy for oscillator phases.')
...
def main():
    ...
    print('=' * 60)
    print('OSCILLATOR-BASED ISING MACHINE (OIM) MAX-CUT SOLVER')
    print('=' * 60)
    print(f'Steps            : {args.steps}')
    print(f'Snapshot frequency: {args.freq}')
    print(f'Seed             : {args.seed}')
    print(f'Coupling mode    : {args.coupling}')
    print(f'K                : {args.K}')
    print(f'Ks               : {args.Ks}  (lambda = {args.Ks / args.K:.4f})')
    print('=' * 60)
    ...
    if args.data:
        ...
        raise ValueError('Provide --data or --random')
    ...
    # ---- Initialise network ----
    print('\nInitialising OIM Max-Cut network...')
    net = OIMMaxCut(W, seed=args.seed, K=args.K, Ks=args.Ks,
                    coupling=args.coupling, init_mode=args.init_mode)
    print(f'✓ Network initialised ({n} oscillators)')
